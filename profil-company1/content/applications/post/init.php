...
function article_approved(){
	...
	while( $row_post = $db->fetch_obj( $sql_post ) ){
		...
		$post .= '<li class="'.$warna.'"><img src="'.get_gravatar($field->user_email).'" style="float:left; width:40px; height:40px; margin-right:5px;" class="radius">'.$row_post->title;
...
function article_comment(){
	...
	while( $row_comment = $db->fetch_obj( $sql_comment ) ){
		...
		$comment .= '<li class="'.$warna.'"><img src="'.get_gravatar($row_comment->email).'" style="float:left; width:40px; height:40px; margin-right:5px;" class="radius">'.$row_comment->author. ', ' .time_stamp($row_comment->time) . '<br>';
