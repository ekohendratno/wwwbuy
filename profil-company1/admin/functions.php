...
<ul class="mainMenu tiptip menuNavJumpFirst"  style="display:block;">
    ...
    <ul class="mainMenuSub" style="right:0;left:auto">
        ...
        <li class="logout"><a id="popup" data-type="confirm" href="?request&load=libs/ajax/logout.php" title="Confirm logout">Keluar</a></li>
...
<div id="dashboard-notification" class="gd">
    ...
    <div class="dn-content-cols">
        <div class="cols" style="background: #f8f8f8;margin-right: 5px;padding: 2px; border-right:1px solid #ddd; padding-left:20px; padding-bottom:10px; text-align:center;">
            ...
            <a href="?admin=single&sys=appearance&go=theme-editor&theme=<?php echo get_option('template');?>" class="button button1 green icon_setting">Custome situs</a>
...
body{
	...
	<?php } if( !empty($style->background->position[0]) && $style->background->position[0] != 'none' ){?>
	background-position: <?php echo $style->background->position[0];?>;
	<?php } if( !empty($style->background->position[1]) && $style->background->position[1] != 'none' ){?>
	background-position: <?php echo $style->background->position[1];?>;
	<?php } if( !empty($style->background->position[2]) && $style->background->position[2] != 'none' ){?>
	background-position: <?php echo $style->background->position[2];?>;
	<?php } if( !empty($style->background->attachment) && $style->background->attachment != 'none' ){?>
