<?php
	...
	$s = mysql_query ("SELECT * FROM `mod_member` ORDER BY `ordering` ASC");
	$i =  0;
	while($data = mysql_fetch_array($s)){
		...
		if ($data['url']== '?action=logout') {
			...
			$images	= $data['mod'] == 1 ? 'mod-member/'.$data['name'].'/images/'.$data['aksi'].'.png' : 'images/'.basename($data['url'],'.php').'.png';
		}
